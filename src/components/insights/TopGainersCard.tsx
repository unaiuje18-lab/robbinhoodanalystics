...
export function TopGainersCard() {
  ...
  const controls = <RangeToggle value={range} onChange={setRange} />;
  ...
  // Same chart at either size — the card's 190px view and the expanded modal.
  ...
  return (
    <>
      ...
      {expanded ? (
        <ChartModal title="Top Gainers" controls={controls} onClose={() => setExpanded(false)}>
          {legend}
          {renderChart(460)}
        </ChartModal>
      ) : null}
